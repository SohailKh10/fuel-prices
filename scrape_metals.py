...
Symbols: the "=X" spot-rate tickers (XAUUSD=X etc.) are unreliable on Yahoo
for server-side/automated requests — they intermittently return an empty
result block with no error, which is what was happening here. The "=F"
futures contracts (COMEX/NYMEX) track spot price closely and respond
consistently, so those are used instead.
"""
import json
import sys
import time
...
SYMBOLS = {
    "XAU": "GC=F",   # COMEX gold futures — tracks spot closely, reliable on Yahoo
